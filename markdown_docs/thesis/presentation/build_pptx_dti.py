...
tb = s.shapes.add_textbox(Inches(0.75), Inches(2.2), Inches(6.6), Inches(2.4))
tf = tb.text_frame; tf.word_wrap = True
...
add_callout(s, "That is an average result. A favourable trial-level effect is compatible with "
               "subgroups that gain nothing — or are harmed.", x=0.75, y=4.75, w=6.6, h=1.0)
...
tb = s.shapes.add_textbox(Inches(7.9), Inches(4.6), Inches(4.4), Inches(0.9))
tf = tb.text_frame; tf.word_wrap = True
p = tf.paragraphs[0]
...
], x=0.75, y=2.2, w=10.9, h=4.9, size=14, gap=13)
