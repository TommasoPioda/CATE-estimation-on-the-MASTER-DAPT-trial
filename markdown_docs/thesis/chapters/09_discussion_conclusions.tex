...
No exploitable heterogeneity emerges. The validation compares the dispersion of the
...
The same conclusion is reached a second time without estimating a single CATE. Chapter
8's guided-enrollment mechanisms move the estimated-CATE plane exactly as designed, and
...
the larger one: the win--win score of Chapter 8 correlates $\rho=+0.91$ with the bleeding
...
Chapter 8's own scope is narrower than its headline result suggests. The main comparison
...
this cohort, so Chapter 8 can say that guided enrollment does not change which real
...
this one, that the guided-enrollment machinery of Chapters 3 and 8 would have something to
...
Two extensions follow directly from Chapter 8's own limitations. The whole-pool
...
Chapter 3 originally set out to make and Chapter 8 could not complete.
