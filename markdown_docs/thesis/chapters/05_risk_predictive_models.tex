% !TEX root = ../Thesis.tex
\section{What This Chapter Produces, and for Whom}
...
propagates directly into the effect estimate: a T-learner cannot be sharper
than the two surfaces it differences. The second use is as a benchmark. The
...
the patient actually received}. That is the quantity Chapter 1 warns against
...
from a property of one algorithm — the same logic that Chapter 6 applies to
the five effect estimators.

\paragraph{Pipeline Preprocessing}
All models are evaluated using a threshold-free 5-fold stratified cross-validation over the
full cohort of 4,579 patients. Within each fold, missing values are imputed using a k-nearest-neighbour 
imputer and features are robustly scaled, with both preprocessing steps fitted exclusively 
on the training data to avoid data leakage. Since event rates range from 0.8% to 11.1%, 
the default 0.5 classification threshold would almost always predict the majority class, 
making confusion-matrix-based metrics uninformative. We therefore assess discrimination without 
fixing an operating threshold, using AUROC and AUPRC. Each patient is scored exactly once by a model 
that did not see them during training; the out-of-fold probabilities are then pooled to obtain a 
single ROC and PR curve, while mean and standard deviation across folds are also reported to 
quantify variability, which is particularly substantial for the rarest endpoints.
...
3-fold cross-validation on the training split. The gains are real but small —
$+0.014$ mean PR-AUC for logistic regression, $+0.026$ for the random forest,
$+0.006$ for gradient boosting — and they do not change any of the
conclusions of this chapter. This is the first appearance of a pattern that
recurs throughout the thesis: on this cohort, tuning moves the numbers by less
than the choice of endpoint does.

\paragraph{Reading AUPRC}
Unlike AUROC, whose chance level is 0.5 for every endpoint, the chance level
of AUPRC is the event rate itself. An AUPRC of 0.06 is therefore excellent for
an endpoint with 1.8\% events and worthless for one with 11.1\%. Every AUPRC
...
\caption{Pooled out-of-fold ROC and precision--recall curves for the soft
voting ensemble, all five endpoints. Every patient is scored once by a model
fitted without them. In the right-hand panel the dotted horizontal lines mark
the chance level of each curve, which is the endpoint's event rate.}
\label{fig:roc-pr}
\end{figure}

The models carry genuine signal for every endpoint: all five curves in Figure~\ref{fig:roc-pr}
sit above the diagonal, and all five AUPRC values sit above their event rate.
The covariates are informative; they are simply not \emph{very} informative.
Even though the models carry some signal they should not be considered clinically
deployable due to their weak-to-moderate performances.


\begin{table}[h]
\centering
\caption{Out-of-fold AUPRC for the soft voting ensemble, with its chance level
(the event rate) and the resulting lift.}
\label{tab:risk-auprc}
\begin{tabular}{lccc}
\toprule
Endpoint & AUPRC & Chance level & Lift \\
\midrule
...
despite having substantially less data. Cardiovascular death (81 events) reaches an AUROC of 0.76 
and myocardial infarction (109) 0.73, compared with 0.62 for bleeding (506) and 0.63 for BARC 2/3/5 (359). 
The same pattern appears in the AUPRC lift, where the rarer non-fatal endpoints show 
...
An independent earlier analysis using gradient-boosted trees and a different validation scheme 
...
events per fold, its AUROC varies substantially across folds, with standard 
...
Stroke illustrates the distinction between finding no predictive signal and 
having too little information to detect one.

\paragraph{What the models look at.}
The feature attributions are consistent with the anomaly rather than
...
oral anticoagulation, prior bleeding history, renal function and the
acute-coronary-syndrome presentation: variables that overlap heavily with the
eligibility criteria through which the cohort was assembled. The two lists
differ in kind, not only in content, and Chapter 7 argues that this difference
is the anomaly's mechanism.
...
death, one panel per model family. The ranking is dominated by continuous
...
panel per model family. The ranking is dominated by variables that also appear
among the trial's high-bleeding-risk eligibility criteria.}
...
arm, so that their difference estimates the effect of the treatment. Three
requirements follow, and they are met here because
they are properties of the risk models, not of the effect estimator.

\paragraph{The same family on both arms.}
Both arms are fitted with a single model family, tuned separately per arm. The
constraint matters more than it looks: differencing a random forest against a
logistic regression produces a surface whose variation reflects the two
algorithms' differing inductive biases as much as any treatment effect, and
that spurious variation is indistinguishable, at the level of the output, from
heterogeneity. Fixing the family removes that failure mode by construction.

\paragraph{Calibrated probabilities.}
Each arm's tuned pipeline is wrapped in a Platt (sigmoid) calibrator, fitted by
internal cross-validation and chosen over isotonic regression because isotonic
fitting is unstable at these event rates. Calibration is irrelevant to
everything in the previous section — it is a monotone transformation, so it
leaves AUROC and AUPRC untouched — and essential here, because the T-learner
does not rank the two probabilities, it \emph{subtracts} them. A systematic
distortion that cancels out of a ranking does not cancel out of a difference.
The out-of-fold Brier score improves substantially on every endpoint and in
both arms, for instance from 0.180 to 0.117 for bleeding under prolonged
therapy and from 0.149 to 0.080 under abbreviated therapy.
...
Bleeding BARC 2/3/5  &  9.2\% & $0.57 \pm 0.02$ &  6.5\% & $0.62 \pm 0.03$ \\
MI                   &  2.1\% & $0.71 \pm 0.06$ &  2.6\% & $0.66 \pm 0.07$ \\
Cardiovascular death &  1.9\% & $0.78 \pm 0.04$ &  1.6\% & $0.71 \pm 0.07$ \\
Stroke               &  1.0\% & $0.64 \pm 0.12$ &  0.5\% & $0.64 \pm 0.13$ \\
\bottomrule
\end{tabular}
\end{table}

The event-rate columns of Table~\ref{tab:per-arm} are worth a moment on their
own, because they are the trial's own result stated without any model. Under
the sign convention of Chapter 4, the unadjusted difference between the arms
is $+4.5$ percentage points on bleeding and $+2.8$ on BARC 2/3/5 — prolonging
causes more bleeding — against differences of $-0.5$, $+0.3$ and $+0.5$
percentage points on MI, cardiovascular death and stroke, which is
noninferiority on the ischemic side within the resolution these event counts
allow. Chapter 6 shows that all five effect estimators recover exactly these
magnitudes. That agreement is the positive control that makes the negative
result of the same chapter interpretable: the estimators see what is there.
...
pass. Cardiovascular death, with a sixth as many events, is predicted best.
...
death, which is not what the wider literature reports, where bleeding risk
scores routinely achieve higher discrimination than the values in
Table~\ref{tab:risk-auroc}. Or the cohort itself might have been assembled in
a way that removes the variation a bleeding model would need. Chapter 7 takes
the third reading, shows that it is testable, and shows that the same property
of the cohort accounts for both this predictive anomaly and the absence of
detectable treatment effect heterogeneity that Chapter 6 is about to report.
Holding the question open until then costs two chapters of suspense and buys
an explanation that owes nothing to the causal estimates in between.

\todo[inline]{Decide whether the earlier eight-endpoint gradient-boosting
report should be cited as a reference or reproduced in an appendix; its
validation scheme differs from the out-of-fold protocol used here.}
