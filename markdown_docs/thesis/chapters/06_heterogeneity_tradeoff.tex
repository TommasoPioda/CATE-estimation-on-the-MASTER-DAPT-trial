...
of the placebo distribution trhough the formula:
\[
...
The BLP (\textbf{Best Linear Prediction slope}) measures whether the model's 
predicted CATE ranks patients by their true treatment effect. It is computed by 
linear regression:

\[
\hat{\psi}_i = \beta_0 + \beta_1 \widehat{\text{CATE}}_i + \epsilon_i,
\]

\noindent where $\hat{\psi}_i$ is the doubly-robust effect estimate for patient $i$ 
and $\widehat{\text{CATE}}_i$ is the model's predicted effect. A slope $\beta_1$ 
significantly different from zero indicates the ranking captures real heterogeneity; 
a slope near or negative zero indicates noise. The BLP results provide a similarly 
weak indication of treatment-effect heterogeneity.
\vspace{10pt}
...
Two features are particularly relevant: the limited variation in bleeding risk
across patients, which may constrain the ability to detect heterogeneity even
despite the 506 observed bleeding events.

and the combination of ischemic
endpoints whose treatment effects differ in direction. These factors provide
a potential explanation for why strong average effects can coexist with
limited evidence of patient-level treatment-effect heterogeneity.